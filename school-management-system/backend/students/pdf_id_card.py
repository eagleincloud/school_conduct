...
def _header_title(school_name: str) -> str:
    """Header title: returns school_name when provided; otherwise, defaults to 'School Name'."""
...
def _draw_photo_cover_box(canv, image_source, x: float, y: float, box_w: float, box_h: float) -> None:
    """object-fit: cover — the photo fills the entire frame; sides/top might be slightly cropped, without leaving white borders."""
    if not image_source:
        return
...
def build_student_id_card_pdf(
    ...
    *,
    ...
    photo_source = get_safe_image_source(student.photo)
    if photo_source:
        try:
